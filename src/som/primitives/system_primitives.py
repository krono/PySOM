...
def _time(ivkbl, frame, interpreter):
    frame.pop() # ignore
    _time = time.time() - interpreter.get_universe().start_time
    frame.push(interpreter.get_universe().new_integer(int(_time * 1000)))

def _ticks(ivkbl, frame, interpreter):
    frame.pop() # ignore
    _time = time.time() - interpreter.get_universe().start_time
    frame.push(interpreter.get_universe().new_integer(int(_time * 1000000)))
